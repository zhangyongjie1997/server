(function() {
  ...
  function createToast(text) {
    if ($(".container-toast").length < 1) {
      let toast = $(
        '<div class="toast-layer"><div class="container-toast">' +
          text +
          "</div></div>"
      );
  ...
  function getOpenId() {
    var dtd = $.Deferred(),
      ...
      redirectUri = encodeURIComponent(location.href.replace(/#.*/, '').replace(/(code|state)=[^&]+[&]?/g, '').replace(/&$/, '')),
      baseUrl = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + appid + "&redirect_uri=" + redirectUri + "&response_type=code&scope=snsapi_base&state=null#wechat_redirect";
    if (openId) {
      dtd.resolve(openId);
    } else if (urlGet.code) {
