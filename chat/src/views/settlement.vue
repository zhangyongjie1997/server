  ...
  <div class="main">
    <transition name="el-fade-in">
      <div v-if="orderStep == 1">
        <el-table v-if="!editShop" key="mainTable" border ref="shopTable" :data="shopList" style="width: 100%">
          <el-table-column prop="name" label="商品名" width="180">
          </el-table-column>
          <el-table-column align="center" prop="price" :formatter="priceFormatter" label="价格" width="180">
          </el-table-column>
          <el-table-column align="center" prop="shopCount" label="数量" width="280">
          </el-table-column>
          <el-table-column :formatter="colSumFormatter" align="center" prop="shopCount" label="总价">
          </el-table-column>
        </el-table>
        <el-table v-if="editShop" ref="shopTableEdit" :data="backupShopList" border style="width: 100%" key="editTable">
          <el-table-column prop="name" label="商品名" width="180">
          </el-table-column>
          <el-table-column align="center" prop="price" :formatter="priceFormatter" label="价格" width="180">
          </el-table-column>
          ...
          <el-table-column :formatter="colSumFormatter" align="center" prop="shopCount" label="总价" width="180">
          </el-table-column>
          <el-table-column label="操作" align="center">
            <template slot-scope="scope">
              <el-button size="mini" type="danger" @click="handleDeleteCol(scope.$index, scope.row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-row class="table_footer">
          ...
          <el-col :span="15" class="text-right" v-if="!editShop">
            <el-button plain type="danger" @click="handleEditShop">编辑</el-button>
          </el-col>
          <el-col :span="15" class="text-right" v-if="editShop">
            <el-button plain type="danger" @click="handleEditShopSave">保存</el-button>
        ...
        <div class="addressInfo">
          ...
          <el-button plain type="warning" @click="orderCommit">提交</el-button>
        </div>
      </div>
    </transition>
...
export default {
  ...
  methods: {
    ...
    orderStatusPolling() {
      let that = this;
      orderStatus({ orderId: this.order.id }).then(res => {
        console.log(res);
        if (res.code != 0) return that.$message.error(res.msg);
    ...
    handleEditShopSave() {
      ...
      editShopSubmit({
        userPhone: this.$store.state.user.phone,
        shopList: JSON.stringify(this.getSubmitShopList())
      }).then(res => {
        if (res.code != 0) {
    ...
    handleDeleteCol(index, row) {
      ...
      this.backupShopList = this.backupShopList.filter(
        item => item.id != row.id
      );
    },
